...
import { ReverseScoreResultSchema } from "@/lib/scoreSchema";
import { REVERSE_SCORING_SYSTEM_PROMPT } from "@/lib/reverseScoringPrompt";
...
// "한 방 프롬프트"는 프로그램 전체 명세라서 일반 프롬프트보다 훨씬 길다
...
/**
 * Claude API 호출 및 응답 파싱 (프롬프트 역설계 4축 채점)
 */
async function callClaudeAPI(
  prompt: string,
  retry: boolean = false
): Promise<ReverseScoreResult> {
  ...
  const message = await anthropic.messages.create({
    ...
    max_tokens: 4000,
    temperature: 0.1,
    system: REVERSE_SCORING_SYSTEM_PROMPT,
    messages: [
      {
        role: "user",
        content: userMessage,
  ...
  // Zod 검증
  const validated = ReverseScoreResultSchema.parse(parsed);

  // total_score 재계산 — 역설계 모드는 가점·감점 없이 4축 합이 곧 총점
  const correctedTotal = Math.max(
    0,
    Math.min(
      100,
      validated.elements.observe.score +
        validated.elements.spec.score +
        validated.elements.edge.score +
        validated.elements.structure.score
    )
  );
  ...
  return {
    ...
    // 답 유출 방지: 역설계 모드는 가점·감점을 쓰지 않음 (모델이 넣어도 무시)
    bonuses: [],
    penalties: [],
  } as ReverseScoreResult;
}
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("Reverse engineering scoring completed", {
      promptLength: trimmedPrompt.length,
    ...
    console.error("Unexpected error in /api/score/reverse", {
