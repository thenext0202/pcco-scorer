...
import { ImageScoreResultSchema } from "@/lib/scoreSchema";
import { IMAGE_SCORING_SYSTEM_PROMPT } from "@/lib/imageScoringPrompt";
...
// 이미지 프롬프트는 텍스트 프롬프트보다 약간 길어질 수 있음
...
/**
 * Claude API 호출 및 응답 파싱 (SSDHR 이미지 프롬프트 채점)
 */
async function callClaudeAPI(
  ...
  const message = await anthropic.messages.create({
    ...
    max_tokens: 4000,
    temperature: 0.1,
    system: IMAGE_SCORING_SYSTEM_PROMPT,
    messages: [
      {
        role: "user",
        content: userMessage,
  ...
  // Zod 검증
  const validated = ImageScoreResultSchema.parse(parsed);

  // total_score 재계산 (Claude의 계산 오류 방지)
  const elementsSum =
    validated.elements.scene.score +
    validated.elements.style.score +
    validated.elements.detail.score +
    validated.elements.hard.score +
    validated.elements.reality.score;

  // 가점은 N/A (null) 제외하고 합산, 최대 +10
  const bonusesSum = Math.min(
    validated.bonuses.reduce(
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log("SSDHR Image scoring completed", {
      promptLength: trimmedPrompt.length,
    ...
    console.error("Unexpected error in /api/score/image", {
