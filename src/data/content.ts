...
export const courses = [
  ...
  {
    id: "course-12",
    title: "구조를 그린다 → 화면을 그린다 — 개발 세계의 지도",
    subtitle: "중급반 1·2강. 매주 점수를 받던 채점기의 속을 펼친다 — 폴더는 왜 갈라졌고, 화면은 무엇으로 만들어지나",
    duration: "각 회차 약 160분 (강의 130여 분 + 숙제 브리핑 + 휴식 10)",
    level: "중급반 (10차 진단 기반 배정) · 12차·14차 연속",
    framework: "개발 지도 3층 · 화면 해부 4단",
    frameworkFull: "지형·골목·문패 / 뼈대·옷·움직임·부품",
    heroQuote: "모든 파일에는 자리가 있고, 그 자리에는 이유가 있다 — 화면은 한 장의 그림이 아니라, 부품과 상태의 조립이다",
    tagline: "중급반 현재 계획 3회: 구조를 그린다(12차) → 화면을 그린다(14차) → 데이터를 그린다(16차) — 숙제 3연쇄 「내 앱 설계 3종 세트」",
    prerequisite: "10차 진단 (중급반 배정) · 1~10차 전체 수강 가정 · 채점 플랫폼 미사용 — 강의 + 숙제(구조도·화면도) · 코드는 띄우지 않는다(14차 한 장 예외)",
    ...
    promises: [
      ...
      "화면을 뼈대·옷·움직임·부품으로 뜯어 보는 눈을 얻고, '화면 하나는 그림 여러 장'(비어 있음·기다림·성공·실패)이라는 상태 감각을 갖는다",
      "내 앱의 구조도(12차)와 화면도(14차)를 종이에 그려 나간다 — 16차 데이터도까지 모으면 「내 앱 설계 3종 세트」"
    ],
    ...
    beforeAfter: {
      ...
      after: {
        ...
        prompt: "버튼[화면] → /api/score[연결] → 채점 기준+Claude[동작, 열쇠는 서버에서만] → 결과 카드[화면] → Supabase[데이터] → 리더보드 / 화면 안쪽에선 버튼[움직임] → 회색 상자[상태] → 결과 카드[부품] → 리더보드[이동]",
        result: "같은 3초를 지형(12차)·화면(14차)·창고(16차)의 눈으로 세 번 본다"
      }
    },

    curriculum: [
      { section: "12차 Part 0 — 중급반 개강: 진단받던 도구가 교재가 된다 · 핵심 메시지", time: "12분", slides: "1~4" },
      { section: "12차 Part 1 — 지형: 주소창의 여정 · 브라우저와 서버 · 프론트 · 백(비밀·신뢰·힘) · DB(동시성·검색·규칙) · API", time: "55분", slides: "5~12", highlight: true },
      { section: "12차 Part 2·3 — 골목: 루트 트리 · 안 만든 파일들(재료 목록·창고·열쇠·문지기) · 이름 정리(러시아 인형) · src 다섯 골목 · 폴더가 곧 주소", time: "45분", slides: "13~19" },
      { section: "12차 Part 4·5 — 좋은 구조의 원칙 5 · 채점 버튼의 여정 · 숙제: 내 앱 구조도", time: "38분", slides: "20~23", highlight: true },
      { section: "14차 Part 0 — 12차 한 장 복습 · 핵심 메시지", time: "15분", slides: "1~3" },
      { section: "14차 Part 1 — 삼형제: 설계도 세 장 · 뼈대(상자 속 상자) · 정체 공개(코드 한 장) · 옷(같은 뼈대 다른 옷) · 움직임(누르면 일어난다) · UI vs UX", time: "52분", slides: "4~11", highlight: true },
      { section: "14차 Part 2 — 부품: 반복에 이름 붙이기 · 빈칸 · 부품 31개 지도 · 사온 부품", time: "33분", slides: "12~15" },
      { section: "14차 Part 3·4 — 상태(화면 하나는 그림 여러 장) · 이동 · 여정 ② 화면판 · 좋은 화면의 원칙 5", time: "40분", slides: "16~19", highlight: true },
      { section: "14차 Part 5 — 숙제: 내 앱 화면도 · 16차 예고(창고 속은 16차에)", time: "10분", slides: "20~22" }
    ],
...
export const courseDetails = [
  ...
  },
  {
    ...
    parts: [
      ...
      {
        title: "Part 5. 13차 — 쓴다: 구조도를 프롬프트로 옮기는 법",
        content: [
          {
  ...
  {
    id: "course-12",
    heroMessage: "중급반은 만드는 날이 아니라 이해하는 날로 시작합니다. 10차에선 소스를 감췄지만, 12차엔 여러분이 매주 점수를 받던 그 채점기의 속 지도를 다 펼칩니다 — 폴더가 왜 그렇게 갈라졌는지, 파일 하나하나가 왜 그 자리에 있는지. 14차엔 그 지도의 「화면」 폴더 안으로 들어가, 매주 눌렀던 화면을 뼈대·옷·움직임·부품으로 뜯습니다. 이미 다 해봤습니다. 오늘은 이름을 배웁니다.",
    promises: [
      ...
      "화면을 뼈대(HTML)·옷(CSS)·움직임(JS)·부품(컴포넌트)으로 나눠 보고, 상태 4장(비어 있음·기다림·성공·실패)을 빠뜨리지 않는 화면도를 그린다",
      "채점 버튼의 3초를 지형(12차)과 화면(14차)의 눈으로 두 번 보고, 16차에 창고(데이터)의 눈으로 완성한다"
    ],
    beforeAfterExample: {
      ...
      after: {
        label: "✅ 지도를 든 사람",
        prompt: "버튼[화면] → 화면 없는 골목 /api[연결] → 채점 기준+Claude[동작 — 열쇠는 서버에서만] → 결과 카드[화면] → 창고[데이터] → 리더보드. 화면 안쪽에선 버튼[움직임] → 회색 상자 셋[기다림 상태] → 결과 카드[부품] → 등록 카드[하단 고정] → 리더보드[이동]",
        result: "폴더마다 이유를, 화면마다 상태를 적을 수 있다 — 그게 곧 설계"
      }
    },
    parts: [
      {
        title: "Part 1. 12차 — 지형: 개발 세계는 크게 어떻게 나뉘나",
        content: [
          {
      ...
      {
        title: "Part 2. 12차 — 골목과 문패: 폴더는 왜 갈라졌고, 파일은 무슨 신분인가",
        content: [
          {
      ...
      {
        title: "Part 3. 12차 — 좋은 구조의 원칙 5 · 채점 버튼의 여정",
        content: [
          {
      ...
      {
        title: "Part 5. 14차 — 상태와 이동: 화면 하나는 그림 여러 장이다",
        content: [
          {
    ...
    checkCard: [
      ...
      { code: "뼈", name: "화면", question: "이 화면의 상자·부품·상태 4장을 다 적었나? 도착 화살표 없는 화면은 없나?" }
    ],
    conclusion: "12차에 우리는 아무것도 만들지 않았습니다. 대신 지도를 얻었습니다 — 모든 파일에는 자리가 있고, 그 자리에는 이유가 있다.\n\n14차엔 그 지도의 화면 폴더를 열었습니다. 화면은 한 장의 그림이 아니라 부품과 상태의 조립이고, 매주 눌렀던 그 버튼은 화면을 세 번 바꿉니다.\n\n같은 3초를 16차엔 창고의 눈으로 봅니다 — 점수와 이름이 어디서 오고 어디에 남는지. 세 장을 모으면 「내 앱 설계 3종 세트」, 그다음에 진짜로 만듭니다.",
    challenge: "12차 숙제는 내 앱 구조도(설계지 4쪽), 14차 숙제는 내 앱 화면도(화면도 4쪽) — 둘 다 종이와 펜이면 충분합니다. 화면도 4쪽의 「이 화면에 보이는 데이터」 칸을 꼭 채워 16차에 가져오세요. 그 칸이 데이터도의 첫 재료입니다.",
    faqs: [
      {
        q: "왜 코드를 안 보여주나요?",
        a: "개발이 처음인 사람에게 코드는 정보가 아니라 소음이기 때문입니다. 지도(12차)와 4단의 눈(14차)이 먼저 생겨야 코드가 '아, 이게 그 상자구나'로 읽힙니다. 14차엔 딱 한 장 — 상자 여섯 개를 아홉 줄로 대조하는 장면만 예외로 두었습니다. 나머지는 다음 시즌에."
      },
      {
        q: "중급반은 세 번으로 끝나나요?",
      ...
      {
        q: "채점 플랫폼은 왜 안 쓰나요?",
        a: "이 두 회차의 산출물은 점수가 아니라 종이 한 장(구조도·화면도)이기 때문입니다. 정답이 하나가 아닌 설계는 기계 채점보다 원칙 5로 스스로 검사하는 편이 낫습니다."
      }
    ]
  }
];
