...
interface ImageScoreResultProps {
  result: ImageScoreResult;
}

const ELEMENT_INFO = {
  scene: { icon: "🎬", name: "장면 (Scene)" },
  style: { icon: "🎨", name: "스타일 (Style)" },
  detail: { icon: "🔍", name: "디테일 (Detail)" },
  hard: { icon: "🔒", name: "강제 규칙 (Hard)" },
  reality: { icon: "⚖️", name: "물리 규칙 (Reality)" },
} as const;

const BONUS_LABEL: Record<string, string> = {
  token_order: "토큰 순서",
  no_conflict: "충돌 없음",
  negative_defined: "양방향 정의 (Negative)",
  json_asset: "JSON 자산화",
  reference_split: "레퍼런스 분리",
};
...
export default function ImageScoreResultView({
  result,
}: ImageScoreResultProps) {
  ...
  const appliedBonuses = result.bonuses.filter((b) => b.points !== null);
  const naBonuses = result.bonuses.filter((b) => b.points === null);
  ...
  return (
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <h3 className="text-xl font-bold">📊 SSDHR 요소별 점수</h3>
        </CardHeader>
