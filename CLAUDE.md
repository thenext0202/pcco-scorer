...
- 현재: R-PCCO(1차), I-MRKO(2차), SSDHR(3차), 바이브 코딩(4차), AI 자동화 이해(5차), Claude Code 실전(6차), PWA(7차), 자동화 설계 PILOT(8차), 진짜 서비스 만들기(9차), 프롬프트 역설계(10차), 그린다·레이어 4겹(11차 초급반 1강), 개발 지도 3층(12차 중급반 1강 — 자료 완성, 랜딩 course-12 미반영), 쓴다·변환표 4줄(13차 초급반 2강 — course-11 Part 5로 랜딩 반영) — 강의 자료 13개
- 채점 모드는 5종: 1~4차(prompt·instruction·image·vibe) + 10차(reverse). 5~9·11차는 랜딩만 (11차는 채점 미사용이 설계 의도 — 워크시트+상호리뷰)
- 11차부터 초급반(11·13·15차)/중급반(12·14·16차) 격주 교차 체계. **13차는 채점 미사용으로 확정**(claude.ai Artifacts 직접 실행, 2026-08-25) — 재제출 루프는 플랫폼에 남아 있어 이후 회차 재활용 가능. 15차는 instruction 재활용 예정(15차 기획 때 재검토) — 새 채점 모드 불필요
