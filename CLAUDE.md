...
- 현재: R-PCCO(1차), I-MRKO(2차), SSDHR(3차), 바이브 코딩(4차), AI 자동화 이해(5차), Claude Code 실전(6차), PWA(7차), 자동화 설계 PILOT(8차), 진짜 서비스 만들기(9차), 프롬프트 역설계(10차), 그린다·레이어 4겹(11차 초급반 1강) — 11개 강의
- 채점 모드는 5종: 1~4차(prompt·instruction·image·vibe) + 10차(reverse). 5~9·11차는 랜딩만 (11차는 채점 미사용이 설계 의도 — 워크시트+상호리뷰)
- 11차부터 초급반(11·13·15차)/중급반(12·14·16차) 격주 교차 체계. 13차는 기존 prompt·vibe 모드 재활용, 15차는 instruction 재활용 예정 — 새 채점 모드 불필요
